...
<table class="table-info align-top no-padding no-margins border">
	...
	<tr>
        ...
        <td class="text-xs uppercase">{{date('d-m-Y',strtotime($despachoORP->dao_fecha_despacho))}}</td>
        <td class="text-center bg-grey-darker text-xs text-white ">Cod. Salida:</td>
        <td class="text-xs uppercase">{{$despachoORP->dao_codigo_salida}}</td>
	</tr>
...
<table class="table-info w-100">
	...
	<tbody>
			<tr class="text-sm">
				...
				<td class="text-center text-xxs uppercase font-bold px-1 py-1">{{ $despachoORP->rece_codigo}}</td>
				<td class="text-center text-xxs uppercase font-bold px-1 py-1">
					{{$despachoORP->rece_nombre}}
				</td>
				<td class="text-center text-xxs uppercase font-bold px-1 py-1">{{ $despachoORP->dao_cantidad }}</td>
